...
class ModsMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		// Built-in mod downloader: opens the GameBanana browser pre-filtered to the
		// "Psych Mod Folders" category (id 28367), same DownloaderState/GameBanana/
		// OnlineMods pipeline already used elsewhere in the online build - no browser
		// window is opened, everything downloads and unzips straight into mods/.
		installButton = new FlxButton(startX, 610, "GET MODS", function()
		{
			FlxG.sound.play(Paths.sound('scrollMenu'), 0.6);
			FlxG.switchState(() -> new online.states.DownloaderState("category:28367"));
		});
		// Bumped up from the original 150x70 so it's a comfortable tap target on
		// touchscreens too, not just a mouse-precision click target. At y=610 with
		// height 90 it bottoms out at y=700, leaving a 20px margin above the 720px
		// canvas edge instead of sitting flush against it.
		installButton.setGraphicSize(190, 90);
		installButton.updateHitbox();
		installButton.color = FlxColor.GREEN;
		installButton.label.fieldWidth = 170;
		installButton.label.setFormat(Paths.font("vcr.ttf"), 18, FlxColor.WHITE, CENTER);
		setAllLabelsOffset(installButton, 2, 30);
		add(installButton);
		// Deliberately NOT pushed into buttonsArray: that array is repositioned every
		// frame in updatePosition() to sit relative to the currently selected mod's
		// alphabet (button.y = mod.alphabet.y + 320), which only makes sense for the
		// per-mod action buttons. This is a persistent, fixed-position button that
		// needs to work even with zero mods installed (no selected mod to anchor to),
		// so it keeps its own fixed (startX, 620) position instead.
		//
		// Also deliberately NOT added to visibleWhenHasMods / visibleWhenNoMods: those
		// two lists get set to opposite bools in changeSelection(), so a button pushed
		// to both would just end up permanently controlled by whichever loop runs last.
